...
Routes are locale-prefixed (`/en/...`, `/uk/...`, `localePrefix: 'always'`) via `next-intl`, with every page living under `src/app/[locale]/`; `src/proxy.ts` handles locale negotiation/redirects, and UI chrome strings live in `messages/en.json`/`messages/uk.json`. Translatable route data (`name`, `summary`, `description`, `startPlace`, `endPlace`, `waymarking`, `bestSeason`) lives in a `RouteTranslation` table keyed by `(routeId, locale)`, resolved per-field to the current locale with English fallback via `src/lib/localize.ts`. Stages are **not** translated: `fromPlace`, `toPlace`, and `notes` are plain English columns on `Stage`. `nameEs` is a permanent, always-shown native-name column on `Route` and is **not** part of this locale system; `countries` stays an untranslated facet key (translated only for display, via the `Countries` message namespace). Only two sample routes (`camino-frances`, `camino-portugues-central`) have full Ukrainian content today — the rest fall back to English by design.

The `20260826211238_add_translation_tables` migration is destructive: it drops the old flat text columns (`name`, `summary`, `description`, `startPlace`, `endPlace`, `waymarking`, `bestSeason` on `Route`; `fromPlace`, `toPlace`, `notes` on `Stage`) with no backfill into the new translation tables. The later `20260828140000_drop_stage_translations` migration reverses the stage half — it drops `StageTranslation` and moves `fromPlace`/`toPlace`/`notes` back to `Stage` as plain columns (existing rows backfilled with `''`). As with any schema change to these models, `npx prisma migrate dev` must be followed by `npx prisma db seed` to repopulate content from `src/data/official-routes.ts`.
